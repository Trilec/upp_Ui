...
{
	String out;
	String header_name = project_name + ".h";
	String source_name = project_name + ".cpp";
	out << "uses\n"
	    << "\tCore,\n"
	    << "\tDraw,\n"
	    << "\tCtrlCore,\n"
                          ...
                          const DesignerProjectExportResult& result)
{
	...
	out << "# " << result.project_name << "\n\n"
	    ...
	    << "- Appearance mode: " << (options.appearance_mode == DesignerAppearanceMode::ExactDesign ? "Exact design" : "Theme-first") << "\n\n"
	    << "- Source structure: " << (options.source_mode == DesignerExportSourceMode::SplitHeaderSource ? "Split header/source" : "Single main.cpp") << "\n\n"
	    << "## Open in TheIDE\n\n"
	    ...
	    << (options.source_mode == DesignerExportSourceMode::SplitHeaderSource
	            ? "- `" + result.project_name + ".h` and `" + result.project_name + ".cpp` contain the generated window class.\n"
	            : String())
	    << (options.include_design_json
	            ? "- `" + result.project_name + ".design.json` is the source design model for regeneration.\n"
	            : "- The package was generated from the Designer model, but the source design JSON was not included.\n");
	return out;
}

static String BuildSplitMainCpp(const String& class_name, const String& project_name)
{
	String out;
	out << "#include \"" << project_name << ".h\"\n\n"
	    << "using namespace Upp;\n\n"
	    << "GUI_APP_MAIN\n"
	    << "{\n"
	    << "\t" << class_name << "().Run();\n"
...
static bool SplitGeneratedCode(const String& code, const String& class_name,
                               String& header_out, String& source_out, String& error)
{
	String class_decl = "class " + class_name + " : public TopWindow {";
	int class_pos = code.Find(class_decl);
	int main_pos = code.Find("GUI_APP_MAIN");
	if(class_pos < 0 || main_pos < 0 || main_pos <= class_pos) {
		error = "Unable to split generated code into header/source.";
		return false;
	}

	String prefix = TrimRight(code.Left(class_pos));
	String class_block = TrimRight(code.Mid(class_pos, main_pos - class_pos));
	String prefix_lines;
	Vector<String> lines = Split(prefix, '\n');
	for(String line : lines) {
		if(line.StartsWith("Ui") && line.Find("::Style Make") >= 0)
			line.Insert(0, "inline ");
		prefix_lines << line << "\n";
	}

	String guard = ToUpper(SanitizeDesignerPackageName(class_name, "GENERATED_DESIGNER_WINDOW"));
	guard.Replace(".", "_");
	guard.Replace("-", "_");
	guard = "_GENERATED_" + guard + "_H_";

	header_out
		<< "#ifndef " << guard << "\n"
		<< "#define " << guard << "\n\n"
		<< TrimRight(prefix_lines) << "\n\n"
		<< class_block << "\n\n"
		<< "#endif\n";
                           ...
                           DesignerProjectExportResult& result)
{
	...
	String out_dir = TrimBoth(options.package_directory);
	...
	if(out_dir.IsEmpty()) {
		result.error = "Package directory is empty.";
		return false;
	}
	result.package_dir = out_dir;
	...
	result.header_path = AppendFileName(result.package_dir, result.project_name + ".h");
	result.source_cpp_path = AppendFileName(result.package_dir, result.project_name + ".cpp");
	result.design_json_path = AppendFileName(result.package_dir, result.project_name + ".design.json");
	...
	String stage_header_path = AppendFileName(stage_dir, result.project_name + ".h");
	String stage_source_cpp_path = AppendFileName(stage_dir, result.project_name + ".cpp");
	String stage_design_json_path = AppendFileName(stage_dir, result.project_name + ".design.json");
	String stage_readme_path = AppendFileName(stage_dir, "README.md");

	DesignerCodeGenOptions gen;
	gen.class_name = result.class_name;
	gen.appearance_mode = options.appearance_mode;
	...
	String header_cpp;
	String source_cpp;
	String split_main_cpp;
	if(options.source_mode == DesignerExportSourceMode::SplitHeaderSource &&
	   !SplitGeneratedCode(main_cpp, result.class_name, header_cpp, source_cpp, result.error)) {
		DeleteFolderDeep(stage_dir);
		return false;
	}
	if(options.source_mode == DesignerExportSourceMode::SplitHeaderSource)
		split_main_cpp = BuildSplitMainCpp(result.class_name, result.project_name);

	bool wrote_sources = options.source_mode == DesignerExportSourceMode::SingleMainCpp
		? WriteExportFile(stage_main_cpp_path, main_cpp)
		: WriteExportFile(stage_main_cpp_path, split_main_cpp) &&
		  WriteExportFile(stage_header_path, header_cpp) &&
		  WriteExportFile(stage_source_cpp_path, source_cpp);
	if(!WriteExportFile(stage_upp_path, upp) || !wrote_sources) {
