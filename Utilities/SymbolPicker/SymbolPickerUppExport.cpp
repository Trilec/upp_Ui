	...
	Vector<String>& warnings)
{
	...
	out << "// Icon: " << display_name << "\n";
	out << "// Source: " << item.source_id << "\n";
	out << "// Style: " << style_text << " | Size: " << px << "\n";
	if(!TrimBoth(item.comment).IsEmpty())
		out << "// Comment: " << item.comment << "\n";
	out << "static const unsigned char " << data_sym << "[] = {\n    ";
	...
	Vector<String>* warnings)
{
	...
	out << "// Project: " << (project.project_name.IsEmpty() ? String("(unnamed project)") : project.project_name) << "\n";
