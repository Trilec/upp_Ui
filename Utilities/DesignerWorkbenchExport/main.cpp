...
// Designer version: v1.0.3 Alpha
// Source design: design_designer.json
// Package: DesignerWorkbenchExport
// UMK path: [PATH_TO_UPP_OR_UMK]
// Exported package: C:\Users\admin\Downloads\DesignerWorkbenchExport
...
UiPanel::Style Makelift_tool_button_panelStyle()
{
	// Source node: lift_tool_button_panel / UiPanel
	// Move this method to GeneratedDesignerTheme.cpp if splitting the generated app.
	// Base role: Subtle. Designer appearance overrides applied below.
	// Designer appearance override for lift_tool_button_panel.
	// Role base: Subtle.
	// Remove this block to return to theme defaults.
...
private:
	...
	void BuildControls()
	{
		...
		header_title_card.SetTitle("Designer").SetSubTitle("").SetContentInset(DPI(4)).SetMediaGap(DPI(9)).SetMediaReserve(DPI(0)).SetMediaMin(DPI(15)).SetMediaAutoFit(false).SetMediaSide(UiAlign::LEFT).SetMediaAlign(UiAlign::CENTER, UiAlign::CENTER);
		...
		lift_tool_button_panel.SetCustomStyle(UiTheme::ResolvePanel(UiRole::Subtle));
		lift_tool_button_panel.SetSizeMin(DPI(0), DPI(0));
		left_tools_layout.SetDirection(UiDirection::H).SetGap(DPI(8), DPI(8)).SetInset(DPI(20)).SetWrap(UiBoxWrap::None);
		right_tool_button_panel.SetCustomStyle(UiTheme::ResolvePanel(UiRole::Subtle));
	...
	{
		// Designer appearance export disabled; role/theme-first output only.
	}
	...
	void BuildLayout()
	{
		...
		leftlayout.Add(lift_tool_button_panel).Fixed(DPI(63)).MinMain(DPI(0)).MinCross(DPI(0)).AlignSelf(UiBoxLayout::Align::Start);
		lift_tool_button_panel.Add(left_tools_layout.SizePos());
	...
	UiPanel lift_tool_button_panel;
