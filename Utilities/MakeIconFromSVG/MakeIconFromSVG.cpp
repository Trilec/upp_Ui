...
bool BuildIconHeaderFromSvg(const SvgIconJob& job, String& error)
{
    ...
    if(job.output_mode == SvgIconOutputMode::IML) {
        ...
        for(int i = 0; i < icons.GetCount(); i++) {
            String compressed;
            String iml_error;
            if(!BuildUppImlPayload(icons[i].image, compressed, &iml_error)) {
                error = "failed to encode image: " + icons[i].input_path + " (" + iml_error + ")";
                return false;
            }
            String iml_text;
            if(!BuildUppImlEntryText(icons[i].token, compressed, icons[i].input_path, icons[i].size, iml_text, &iml_error)) {
                error = "failed to format IML entry: " + icons[i].input_path + " (" + iml_error + ")";
                return false;
            }
            iml_append << iml_text << "\n";
