...
static bool ParseWorkingRangeDouble(const String& text, double& out)
{
    const String value = TrimBoth(text);
    if(value.IsEmpty())
        return false;
...
{
    return "property.numeric-double-working-range";
}
...
{
    return Format("%.17g:%.17g", minimum, maximum);
}
                                            ...
                                            double& minimum, double& maximum)
{
    ...
    double hi = 0.0;
    if(parts.GetCount() != 2 ||
       !ParseWorkingRangeDouble(parts[0], lo) ||
       !ParseWorkingRangeDouble(parts[1], hi) || hi <= lo)
        return false;
    minimum = lo;
    maximum = hi;
    return true;
...
class PropertyWorkingRangeDoubleEditor : public PropertyValueEditor {
public:
    PropertyWorkingRangeDoubleEditor()
    {
        Add(edit_);
        Add(slider_);
        Add(toggle_);
    ...
    {
        enabled_ = item.enabled && !item.read_only;
        legal_minimum_ = IsNumber(item.minimum) ? (double)item.minimum : -DBL_MAX;
        legal_maximum_ = IsNumber(item.maximum) ? (double)item.maximum : DBL_MAX;
        step_ = IsNumber(item.step) && (double)item.step > 0.0 ? (double)item.step : 1.0;
        decimals_ = max(0, item.decimals);

        double working_minimum = 0.0;
        double working_maximum = 0.0;
        working_valid_ = PropertyEditorParseWorkingRangeVariant(
            item.editor_variant, working_minimum, working_maximum);
        if(working_valid_) {
            working_minimum_ = max(working_minimum, legal_minimum_);
        ...
        if(!working_valid_ && legal_maximum_ > legal_minimum_ &&
           legal_minimum_ != -DBL_MAX && legal_maximum_ != DBL_MAX) {
            working_minimum_ = legal_minimum_;
            working_maximum_ = legal_maximum_;
            working_valid_ = true;
        }
        ...
        if(working_valid_) {
            slider_.SetRange(working_minimum_, working_maximum_);
            slider_.SetStep(step_);
            slider_.Tip(Format("Working range %.*f–%.*f. Switch to numeric entry for values outside this range.",
                               decimals_, working_minimum_, decimals_, working_maximum_));
        }
        slider_.Enable(enabled_ && working_valid_);
        toggle_.Show(working_valid_);
        toggle_.Enable(enabled_);
...
private:
    class CommitFloatEdit : public UiFloatEdit {
    public:
        typedef CommitFloatEdit CLASSNAME;

        CommitFloatEdit()
        {
            UiFloatEdit::WhenAction = [=] { EmitCommit(); };
        }

        void LostFocus() override
        {
            UiFloatEdit::LostFocus();
            EmitCommit();
        }
    ...
    private:
        void EmitCommit()
        {
            if(!GetText().IsEmpty() && !IsInputComplete())
                return;
            const Value value = GetData();
            if(!has_baseline_ || value != baseline_) {
                baseline_ = value;
    ...
    void SyncSliderFromEdit()
    {
        ...
        if(!IsNumber(value))
            return;
        const double authored = (double)value;
        slider_.SetValue(minmax(authored, working_minimum_, working_maximum_));
    }
    ...
    bool slider_mode_ = false;
    bool working_valid_ = false;
    double legal_minimum_ = -DBL_MAX;
    double legal_maximum_ = DBL_MAX;
    double working_minimum_ = 0.0;
    double working_maximum_ = 100.0;
    double step_ = 1.0;
    int decimals_ = 3;
};
...
void RegisterPropertyEditorWorkingRangeEditors(PropertyEditorFactory& factory)
{
    ...
    factory.RegisterCustom(PropertyEditorWorkingRangeDoubleId(), [] {
        return One<PropertyValueEditor>(new PropertyWorkingRangeDoubleEditor);
    });
}
