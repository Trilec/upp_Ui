...
This guide complements `03_UI_MODEL_GUIDE.md`. The model guide defines ownership,
request-first mutation, stable identity and callbacks; this guide defines the
performance rules that keep those controls usable with very large logical models.

## Core contract

Model-backed controls must treat logical item count and visual control count as
separate things.
...
3. **Viewport work is bounded.** Normal paint, hover and hit testing must be
   proportional to visible/overscan content, not the index of the visible item or
   the total item count.
4. **Geometry has a direct fast path.** Uniform rows/cells use arithmetic mapping
...
5. **Model changes carry useful scope.** `UiModelChange` insert/erase/update/move
...
6. **Paint is render-only.** Expensive decoding, loading, network/disk work and
   semantic mutation never happen from `Paint()`.
7. **Lazy assets follow the viewport.** Views may expose visible/overscan ranges;
...
8. **Bulk user operations may be bulk.** Explicit operations such as Select All,
   copying a large range, exporting, filtering or rebuilding a changed tree can
   be O(N). Ordinary scrolling and painting may not be O(N).
9. **Large logical extent must not overflow geometry.** Shared extent helpers use
...
Its V1/high-scale geometry is intentionally uniform:
...
`WhenPaintItem` is render-only customization. Setting `handled = true` replaces
only the default item painter; it does not transfer selection, scrolling or
model authority to the callback.

Variable-height/masonry layout is intentionally not part of the first contract.
If added later it must declare its geometry/cache cost rather than weakening the
uniform fast path.
...
A single visible-row `UI_MODEL_UPDATE` may repaint that row without treating the
change as a structural rebuild. Insert/erase/move/reset may require broader layout
work because logical row positions and scroll extent can change.
...
The current tree remains a separate scale-audit target for that ordinary paint
path. Lazy children remain model/owner state; loading placeholders are visual
state, not a second authoritative tree.
...
Very large spatial graphs should continue toward viewport culling/spatial lookup
so pan, zoom, paint and hit testing depend on spatially relevant objects rather
than blindly scanning the entire graph. Embedded node controls remain exceptional
interactive content, not the normal graph representation.

## Validation

`Utilities/UiModelViewPerformanceTest` uses deterministic 100,000-item probes.
The tests verify geometry and visit counts rather than fragile elapsed-time
thresholds. In particular they prove that jumping to the final item does not make
ordinary List/Gallery paint visit the preceding 99,999 items.
...
selection painting, lazy preview callbacks and resize are common regression
points.
